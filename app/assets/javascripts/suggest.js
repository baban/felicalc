...
Suggest.Local.prototype = {
  initialize: function(input, suggestArea, candidateList, mother) {
    this.mother = mother ? mother : undefined; // オブジェクトを埋め込んだcoAccessオブジェクト
  ...
  setOptions:function(options){ Suggest.copyProperties(this, options); },
  inputBlur:function(){
    this.changeUnactive();
    this.oldText = this.getInputText();
  ...
  isMatch:function(value, pattern){
    ...
    if(this.displayAuto && pattern.length == 1 && pattern[0] == '') return this._escapeHTML(value[0]);
    ...
    if(pattern != value[0]){
      for(var i = 0; i<value.length ; ++i){
        ...
        for(var j = 0 ; j < pattern.length ; ++j){
          if(pattern[j]=='') continue; // これはマッチしたと扱う。
  ...
  createSuggestArea:function(resultList){
    ...
    var counter = 0, etcExist = false;
    
    // 上の!==ではリセットされていなかったりするんだな
    if(this.getPageMax() < this.pageCounter) this.pageCounter = 0;
  ...
  getInputText:function(){ return this.input.value; },
  setInputText:function(text){ this.input.value = text; },
  // key event
  keyEvent: function(event) {
    if(!this.timerId) this.timerId = setTimeout(this._bind(this.checkLoop), this.interval)
    // データの内容を更新
    var self = this.mother;
    var inp = this.input;
    
    if(this.dispAllKey && this.getInputText() == '' && 
      ...
      // オートコンプリート完了。値をサーバーに送信
      (function( p, e ){
        if(self.table[p.y-1][self.tableHeader[p.x].name] == inp.value) return;
  ...
  // 最終ページは何ページ目なのか
  getPageMax:function(){ return pageMax = Math.ceil(this.resultForRedraw.length/this.dispMax)-1; },
  // 次ページが存在するのか。
  isNextPageExist:function(){ return this.resultForRedraw.length!=0 && this.pageCounter < this.getPageMax(); },
  // 前ページが存在するのか
  isPrevPageExist:function(){ return this.pageCounter>=1; },
  ...
  listMouseOut:function(event, index) {
    ...
    (index == this.activePosition) ? this.setStyleActive(element) : this.setStyleUnactive(element);
  },
  setStyleActive:function(element){
    element.className = this.classSelect;
  },
  setStyleUnactive:function(element){ element.className = ''; },
  setStyleMouseOver:function(element){ element.className = this.classMouseOver; },
  moveEnd:function(){
...
/*-- Suggest.LocalMulti ---------------------------------*/
Suggest.LocalMulti = function(){ this.initialize.apply(this, arguments); };
